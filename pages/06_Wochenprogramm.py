    ...
    for x in vortragsreihen:
        ...
        with co4:
            ...
            if submit:
                ...
                st.switch_page("pages/07_Vortrag.py")
        with co3:
            with st.expander(f"**{tools.repr(collection, x['_id'], False, False)}**"):
                with st.popover('Veranstaltungsreihe löschen'):
                    ...
                    with colu1:
                        submit = st.button(label = "Ja", type = 'primary', key = f"delete-{x['_id']}", disabled = True if x['_id'] == st.session_state.leer[collection] else False)
                    if submit:
